...
int SoH = 100, SoC = 100; //Battery voltage, current, state of health, state of charge
...
void displayCellInfo(float cell_arr[]){  
  ...
  for(int i = 0; i < MAX_CELLS; i++){
    if(cell_arr[i] < 10){ //If cell voltage is less than 10, add a 0 to the front
      snprintf(buf, 12, "C0%d:0%.2fV", i+1, cell_arr[i]); 
      if (9 <= i){ //If cell number is 10 or greater, shift text over
        snprintf(buf, 12, "C%d:0%.2fV", i+1, cell_arr[i]); 
      }
    }
    else{ //If cell voltage is greater than 10, don't add a 0 to the front
...
//Takes a uint16_t and displays it on the screen
void displayBatInfo(char type, float data){
  int x = 0, y = 0;
  char *vbuf = (char*)malloc(12); //Allocate memory for the char array
  switch(type){
    case 'V': case 'v': case '0': //Voltage
      x = 5;
      y = 120;
      snprintf(vbuf, sizeof(vbuf), "Volts:%.2fV", data);
      break;
    case 'C': case 'c': case '1': //Current
      x = 15;
      y = 140;
      snprintf(vbuf, sizeof(vbuf), "Amps:%.2fA", data);
      break;
    case 'H': case 'h': case '2': //State of Health
      x = 25;
      y = 160;
      snprintf(vbuf, sizeof(vbuf), "SoH:%d%c", data, '%');
      break;
    case 'S': case 's': case '3': //State of Charge
      x = 25;
      y = 180;
      snprintf(vbuf, sizeof(vbuf), "SoC:%d%c", data, '%');
      break;
    default :
